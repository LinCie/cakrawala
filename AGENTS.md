...
- **Styling**: Tailwind CSS v4 via `@tailwindcss/vite` plugin — there is no `tailwind.config.*`. All design tokens live in `src/styles/global.css` using the `@theme` directive. **Use Tailwind for ALL styling. Plain CSS is FORBIDDEN except in the narrow cases listed below. If Tailwind can express it, you MUST use Tailwind. Plain CSS is only permitted when: (1) Tailwind cannot render the needed style at all (e.g., complex animations, SVG filters, hacky workarounds), AND (2) you document the specific technical reason in a comment above the style block. Even in those rare cases, prefer Tailwind custom utilities (`@utility`) in `global.css` over inline `<style>` blocks or separate CSS files. Violating this rule is a serious protocol breach.**
